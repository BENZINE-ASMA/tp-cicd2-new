...
def create_app(
    ...
    def get_postcode(postcode):
        res = es.search(
            index=elastic_index, query={"term": {"id": {"value": postcode}}}
        )
        if len(res["hits"]["hits"]) == 0:
            raise HTTPException(
                status_code=404, detail="postcode information not found"
            )
    ...
    # SUPPRIMÉ : L'endpoint /debug avec eval()
    # est une FAILLE DE SÉCURITÉ MAJEURE
